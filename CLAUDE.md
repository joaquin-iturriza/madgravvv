...
| Python env | self-contained `.venv/` from `module load Programming_Languages/anaconda/3.12` then `python -m venv .venv`. **Install torch from the cu124 index FIRST** — the default wheel is CUDA 13, which **dropped Volta (sm_70) → will not run on V100**; cu124 ships sm_70 + sm_90. Then `pip install -e ".[dev,deep,gw]"`. `scripts/setup_env.sh` does this in order. |
